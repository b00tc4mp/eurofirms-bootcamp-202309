...
// Definimos una función llamada retrieveUser que recibe una función de devolución de llamada (callback)
function retrieveUser(callback) {
    ...
    // Realizamos una solicitud al servidor para obtener la información del usuario autenticado
    ...
    .then(res => {
        ...
        // Si la respuesta es exitosa, procesamos el cuerpo JSON para obtener la información del usuario
        res.json()
            .then(body => callback(null, body)) // Llamamos a la devolución de llamada con la información del usuario
            .catch(error => callback(new SystemError(error.message))) // Manejamos posibles errores en el procesamiento del cuerpo JSON
    })
    .catch(error => callback(new SystemError(error.message))) // Manejamos posibles errores en la solicitud
}

// Exportamos la función retrieveUser para que pueda ser utilizada en otras partes del código
