function Home(props) {
    ...
    function handleDeletePostClick(postId) {
        try {
            deletePost(loggedInEmail, postId, posts)
        ...
        {posts !== null ? <div id="posts-list" aria-label="Posts list" className="view">
            {posts.toReversed().map(function (post, index) {
                ...
                return <article key={post.id} className="post">
                    ...
                    <button className="button delete-button" onClick={handleBeforePostDeleteClick}>{(post.author === loggedInEmail ? 'Delete' : null)}</button>
                </article>
            })}
        </div> : null}
    </div>
